...
The alluvial plot represents the change of students' grades over the academic year(From the first period to the final period). Most students get same letter grade for G1, G2 and G3 which implies that if a student perform well in an exam, he/she is likely to get a high grade in the next exam. In this case, we believe that students' grades in different periods are high correlated. We can verify this by compute the correlations. This time, we will use `Math_G1`, `Math_G2`, `Math_G3`, `Por_G1`, `Por_G2` and `Por_G3` which are numeric grade instead of letter grade.
```{r}
...
The pair plot reveals that there are strong correlations within math/portuguese grades(approch 1) in difference period.
